...
px_void PX_ApplicationRender(PX_Application *pApp,px_dword elapsed){
	...
	//绘制文本
	PX_FontModuleDrawText(pRenderSurface,&pApp->fm, startX, startY, PX_ALIGN_LEFTTOP, "请选择要分析的文件:", PX_COLOR(255, 255, 255, 200));
	PX_FontModuleDrawText(pRenderSurface,&pApp->fm, startX, startY + 32 * 2, PX_ALIGN_LEFTTOP, "现有关键词:", PX_COLOR(255, 255, 255, 200));

	//绘制提示信息
	sprintf(content, "已选择的文件:%s", filePath);
